...
def get_local_difference(neighbor_list):
    property_list = ("atomic number", "atomic weight", "period number", "group number", "electronegativity",
                     "covalent radius", 'lattice constant', 'melting point', 'VEC', 'vacancy formation energy',
                     'shear_modulus', 'bulk_modulus', 'young_modulus', 'hardness')
    properties = {'atomic number': [22, 40, 72, 41],
                  ...
                  'shear_modulus': [171.664, 153.100, 178.922, 204.376],
                  'bulk_modulus': [251.736, 221.150, 239.489, 299.68],
                  'young_modulus': [419.612, 373.183, 429.745, 511.449],
                  'hardness': [22.929, 21.682, 26.559, 27.213],
                  'shear_modulus_v': [174.267, 155.440, 175.548, 194.556],
                  'bulk_modulus_v': [243.689, 214.344, 232.487, 283.809],
                  'young_modulus_v': [422.168, 375.540, 420.744, 475.104],
                  'hardness_v': [24.655, 23.292, 26.598, 25.071],
                  }
    feature = []
...
data_vac = pd.read_excel('mechanical_properties.xlsx')
G = data_vac.iloc[:, -4]
B = data_vac.iloc[:, -3]
E = data_vac.iloc[:, -2]
C44 = data_vac.iloc[:, -1]
...
feature = pd.concat([compositions, neighbor_1NN, neighbor_3NN, neighbor_5NN,
                     ...
                     AGNI, GSF, G, B, E, C44], axis=1)
feature.index = index
feature.to_excel('features_mechanical.xlsx', index=index)
